...
walk_left_sprites = [pygame.image.load(f"sprites/chicka_left_{i}.png") for i in range(1, 3)]
walk_right_sprites = [pygame.image.load(f"sprites/chicka_right_{i}.png") for i in range(1, 3)]
static_character = pygame.image.load("sprites/chicka_static.png")
...
guitar_sprite = pygame.image.load("sprites/cake.png")
...
new_size_guitar = (int(guitar_sprite.get_width() * 0.4), int(guitar_sprite.get_height() * 0.4))  # Измените коэффициенты по вашему усмотрению
...
pygame.mixer.music.load("music/minigame_ambient_2.mp3")
...
while True:
    ...
    if not flag_activated and character_x == 310 and character_y == 760:
        flag_activated = True
    ...
    if change_children_sprite:
        screen.blit(guitar_sprite, (178, 950))
