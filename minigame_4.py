...
walk_left_sprites = [pygame.image.load(f"sprites/foxy_left_{i}.png") for i in range(1, 3)]
walk_right_sprites = [pygame.image.load(f"sprites/foxy_right_{i}.png") for i in range(1, 3)]
static_character = pygame.image.load("sprites/foxy_static.png")
...
guitar_sprite = pygame.image.load("sprites/cup.png")
...
new_size_guitar = (int(guitar_sprite.get_width() * 0.2), int(guitar_sprite.get_height() * 0.2))  # Измените коэффициенты по вашему усмотрению
...
pygame.mixer.music.load("music/minigame_ambient_4.mp3")
...
while True:
    ...
    if not flag_activated and character_x == 305 and character_y == 769:
        flag_activated = True
    ...
    if change_children_sprite:
        screen.blit(guitar_sprite, (150, 915))
