...
static_character = pygame.transform.scale(static_character, (static_character.get_width() * character_scale,
...
new_size_guitar = (int(guitar_sprite.get_width() * 0.2), int(guitar_sprite.get_height() * 0.2))
...
while True:
    ...
    if flag_activated:
        ...
        if elapsed_time >= 5:
            cur.execute('''UPDATE minigames SET minigame4 = 'unlocked' ''').fetchall()
            con.commit()
            con.close()
            import night4
            sys.exit()
