...
walk_left_sprites = [pygame.image.load(f"sprites/fredy_left_{i}.png") for i in range(1, 3)]
walk_right_sprites = [pygame.image.load(f"sprites/fredy_right_{i}.png") for i in range(1, 3)]
static_character = pygame.image.load("sprites/fredy_static.png")
...
static_character = pygame.transform.scale(static_character, (
    static_character.get_width() * character_scale, static_character.get_height() * character_scale))
sad_children_sprite = pygame.transform.scale(sad_children_sprite, (
    sad_children_sprite.get_width() * character_scale2, sad_children_sprite.get_height() * character_scale2))
happy_children_sprite = pygame.transform.scale(happy_children_sprite, (
    happy_children_sprite.get_width() * character_scale2, happy_children_sprite.get_height() * character_scale2))
confetti_image = pygame.image.load("sprites/confeti.png")
confetti_image = pygame.transform.scale(confetti_image, (300, 300))
...
class Particle:
    ...
    def draw(self):
        screen.blit(confetti_image, (self.x, self.y))


# Функция для генерации частиц конфетти
...
pygame.mixer.music.load("music/minigame_ambient_5.mp3")
...
while True:
    ...
    if not flag_activated and character_x == 300 and character_y == 768:
        flag_activated = True
    ...
    if flag_activated:
        ...
        if elapsed_time >= 5:
            cur.execute('''UPDATE minigames SET minigame5 = 'unlocked' ''').fetchall()
            con.commit()
            con.close()
            import night5
            sys.exit()
